...
lemma "A \<subseteq> M \<and> B \<subseteq> M \<longrightarrow> A \<inter> B \<subseteq> M"
  by (simp add: inf.coboundedI2)
...
definition latest_message :: "state \<Rightarrow> (validator \<Rightarrow> state)"
  where
    "latest_message \<sigma> v = {m \<in> from_sender (v, \<sigma>). later_from (m, v, \<sigma>) = \<emptyset>}"

lemma (in Protocol) latest_message_type :
  "\<forall> \<sigma> v. \<sigma> \<in> \<Sigma> \<and> v \<in> V \<longrightarrow> latest_message \<sigma> v \<subseteq> M"
  using latest_message_def state_is_subset_of_M by auto
...
lemma (in Protocol) latest_estimates_type :
  ...
  using M_type Protocol.latest_message_type Protocol_axioms latest_estimates_def by fastforce
...
definition latest_messages_from_non_equivocating_validators :: "state \<Rightarrow> validator \<Rightarrow> message set"
  where
    "latest_messages_from_non_equivocating_validators \<sigma> v = (if is_equivocating \<sigma> v then \<emptyset> else latest_message \<sigma> v)"
...
lemma (in Protocol) latest_messages_from_non_equivocating_validators_type :
  ...
  by (simp add: latest_message_type latest_messages_from_non_equivocating_validators_def)
