...
interpretation gp: GhostParams genesis "\<Union> (B_i (genesis, block_generator) `\<nat>)" block_generator W for genesis B block_generator W
  using GhostParams_def by blast 

term gp.estimator

interpretation ap: AbstractProtocol V gp.W t \<Sigma> M B gp.estimator for V W t \<Sigma> M C \<epsilon>
