...
lemma non_equivocating_validators_have_at_most_one_latest_message:
  (* "\<forall> v \<in> v_set. v \<notin> equivocating_validators s \<Longrightarrow> card (latest_message s v) \<le> 1" *)
  (* prove the contrapositive *)
  "\<forall> v \<in> v_set. card (latest_message s v) > 1 \<Longrightarrow> v \<in> equivocating_validators s"
